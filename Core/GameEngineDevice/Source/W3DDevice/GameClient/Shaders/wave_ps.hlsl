...
sampler2D BumpSampler       : register(s0);   // U8V8, signed (du, dv)
sampler2D ReflectionSampler : register(s1);   // the mirrored scene
...
float4 main(float4 color     : COLOR0,
            ...
            float2 reflectUV : TEXCOORD1) : COLOR
{
    ...
    float2 duv = tex2D(BumpSampler, bumpUV).rg;
    ...
    float4 reflection = tex2D(ReflectionSampler, reflectUV + offset);
