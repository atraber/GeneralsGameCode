// Unit vertex shader -- shared body.
//
// Compiled twice, because a vertex shader may only declare inputs its vertex format
// actually supplies and the mesh path has two shapes of it:
//
//   unit_vs.hlsl      -- one mesh coordinate set  (D3DFVF_TEX1 and up)
//   unit_uv2_vs.hlsl  -- two, defining UNIT_VS_UV2 (D3DFVF_TEX2 and up)
//
// The same reason unit_prelit_vs exists as a separate shader rather than a branch:
// reading an input register the declaration does not supply is undefined, so the
// choice has to be made at bind time against the format in the stream. The wrapper
// picks the variant, and only claims a draw for the second one when the FVF really
// carries a second set.
//
...
//     4 = the mesh's *second* coordinate set (D3DTSS_TCI_PASSTHRU | 1), two-set
//         variant only -- the wrapper never asks for it of the one-set shader.
...
#ifdef UNIT_VS_UV2
    // The mesh's second coordinate set. Declared only by the two-set variant: the
    // format in the stream has to supply every input a shader declares.
...
float4 Select_TexGen_Source(float mode, float2 meshUV, float2 meshUV2, float3 viewPos,
                            float3 viewNormal, float3 reflection)
{
    ...
    // The second mesh coordinate set is padded exactly as the first is, and for the
    // same reason: a 2-D coordinate set reaches the fixed-function texture matrix as
    // (u, v, 1, 0), which is the 1 in .z that every 2-D mapper's translation in
    // _31/_32 is picked up by. The detail pass this mode exists for carries such a
    // matrix on three draws in four.
    float w4 = saturate(1.0 - abs(mode - 4.0));
    return w0 * float4(meshUV, 1.0, 0.0)
         + w1 * float4(viewPos, 1.0)
         + w2 * float4(viewNormal, 1.0)
         + w3 * float4(reflection, 1.0)
         + w4 * float4(meshUV2, 1.0, 0.0);
}
...
VS_OUTPUT main(VS_INPUT input)
{
    ...
    // The one-set variant has no second set to offer. Handing it the first would be
    // worse than declining the draw: it renders silently, with the detail texture
    // wrapped by the wrong coordinates. The wrapper never asks -- mode 4 is only ever
    // written for the variant that declares the input -- and passing zero here keeps
    // that true in the shader as well as in the gate.
#ifdef UNIT_VS_UV2
    float2 meshUV2 = input.texcoord2;
#else
    float2 meshUV2 = float2(0.0, 0.0);
#endif

    float4 gen0 = Select_TexGen_Source(TexGenCtl.x, input.texcoord, meshUV2, viewPos, viewNormal, reflection);
    float4 gen1 = Select_TexGen_Source(TexGenCtl.y, input.texcoord, meshUV2, viewPos, viewNormal, reflection);
