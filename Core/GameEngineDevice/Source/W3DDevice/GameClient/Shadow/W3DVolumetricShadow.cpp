...
void W3DVolumetricShadow::RenderMeshVolume(Int meshIndex, Int lightIndex, const Matrix3D *meshXform)
{
	...
	DeclaredTechniqueClass labelDraws(MESH_TECHNIQUE_UNCLASSIFIED, "volumetricShadow");
	Geometry *geometry;
	Int numVerts, numPolys, numIndex;

	// The device itself, because what is left here is the direct draw: this binds its own
	// stream and index buffer and submits from them. Announced with Prepare_Direct_Draw
	...
	// WORLD_CHANGED, which a write straight at the device does not raise -- so the tracked
	// matrix went stale and the next draw that did not set its own world drew with this
	// one. The device write stays because the volume is drawn from an FVF, and
	// fixed-function vertex processing reads D3DTS_WORLD off the device, not off a constant.
