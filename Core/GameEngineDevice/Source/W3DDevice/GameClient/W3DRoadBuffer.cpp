...
// W3DRoadBuffer::loadLit4PtSection and loadLitRoadsInVertexAndIndexBuffers stood here: a
// second copy of the road geometry, re-tessellated and re-coloured on the CPU so that
// dynamic lights would show on roads. Its one call site was already inside an #if 0 (see
// drawRoads), so it had not run in a very long time; road_ps samples the cluster grid now,
// which is the same answer per pixel and needs no second vertex buffer.
...
void W3DRoadBuffer::loadRoadsInVertexAndIndexBuffers()
{
	...
	DX8IndexBufferClass::WriteLockClass lockIdxBuffer(m_roadTypes[m_curRoadType].getIB(), s_dynamic?GFX_MAP_WRITE_DISCARD:GFX_MAP_WRITE);
	DX8VertexBufferClass::WriteLockClass lockVtxBuffer(m_roadTypes[m_curRoadType].getVB(), s_dynamic?GFX_MAP_WRITE_DISCARD:GFX_MAP_WRITE);
