...
//
// HeightMapRenderObjClass::On_Frame_Update lived here, and with it doTheDynamicLight,
// updateVBForLight and updateVBForLightOptimized (roughly 400 lines, above and below this
// point). Every frame they walked the scene's dynamic lights, worked out which heightmap
// vertex-buffer tiles a light had entered or left since the previous frame, LOCKED those
// tiles and re-wrote the diffuse colour of every vertex inside them on the CPU -- a linear
// distance ramp times N.L off the heightfield, added to the static bake.
//
// C7 replaced all of it with terrain_ps sampling the cluster grid per pixel. The terrain no
// longer re-uploads geometry because a light moved, which is where the saving is; and the
// light is no longer quantised to the vertex grid, which is where the look is. The base
// class's On_Frame_Update is empty, so the override is gone rather than left forwarding.
//
// Gone with it: MAX_ENABLED_DYNAMIC_LIGHTS (BaseHeightMap.h) and W3DDynamicLight's
// m_priorEnable / m_processMe / m_minX..m_maxY / m_prevMinX..m_prevMaxY bookkeeping, which
// existed only to answer "which tiles does this light dirty this frame" and had no other
