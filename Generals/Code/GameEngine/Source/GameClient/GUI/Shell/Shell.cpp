...
void Shell::showShell( Bool runInit )
{
	...
	if(!TheGlobalData->m_initialFile.isEmpty() || TheGlobalData->m_initialReplayFile.isNotEmpty() || !TheGlobalData->m_simulateReplays.empty())
	{
		return;
	}
...
void Shell::showShellMap(Bool useShellMap )
{
	...
	if (TheGlobalData->m_initialFile.isNotEmpty() || TheGlobalData->m_initialReplayFile.isNotEmpty() || !TheGameLogic || !TheGlobalData->m_simulateReplays.empty())
		return;
